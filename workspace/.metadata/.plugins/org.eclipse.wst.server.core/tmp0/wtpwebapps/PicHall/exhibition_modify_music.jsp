...
<body class="is-preload">
	...
	<c:set var="musicBeanList" value="${requestScope.musicBeanList}" />
	<c:set var="musicSeqList" value="${requestScope.musicSeqList}" />
		...
		<div id="main">
			...
			<div class="col-12">
				<form name="musicForm" method="post"
					action="${pageContext.request.contextPath}/exhibition/ExhibitionModifyFinishOk.ex">
					...
					<input name="exhibition_seq" value="${requestScope.exhibition_seq}" type="hidden"/>
									...
									<tr>
										...
										<td>
											<div id="check_box" class="col-12 checkbox">
												...
												<c:forEach var="musicSeq" items="${musicSeqList}">
													<c:if test="${musicSeq eq musicBean.getMusic_seq()}">
												...
												<c:choose>
													<c:when test="${check eq true}">
														<input type="checkbox" name="select" id="select${i}"
															value="${musicBean.getMusic_seq()}" checked>
														<label for="select${i}" style="font-family: inherit"></label>
													</c:when>
													<c:otherwise>
														<input type="checkbox" name="select" id="select${i}"
															value="${musicBean.getMusic_seq()}">
														<label for="select${i}" style="font-family: inherit"></label>
													</c:otherwise>
												</c:choose>
											</div>
